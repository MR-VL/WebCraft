...
import {foliage_sdfs} from './foliage-defs.js';
...
export const voxel_block_builder = (() => {
    ...
    class _VoxelBuilderThreadedWorker {
        ...
        CreateTerrain_() {
            ...
            const xp = (GameDefs.skipExteriorBlocks ?
                this.params_.dimensions.x : this.params_.dimensions.x + 1);
            const zp = (GameDefs.skipExteriorBlocks ?
                this.params_.dimensions.x : this.params_.dimensions.x + 1);
                ...
                for (let x = -1; x < this.params_.dimensions.x + 1; x++) {
                    for (let z = -1; z < this.params_.dimensions.z + 1; z++) {
                        ...
                        for (let y = 0; y < 100; y++) {
                            ...
                            if (res) {
                                let roll = 0;
                                if (res == 'tree_leaves' && !GameDefs.skipFoliageNoise) {
        ...
        Rebuild() {
            ...
            const oceanVoxels = !GameDefs.skipOceans ? this.CreateOcean_(terrainVoxels) : {};
